...
def render(
    ...
    # Auto-reduce scale if total pixel count would exceed Playwright's limit (~150MP)
    max_pixels = 150_000_000
    total_pixels = vp_width * scale * vp_height * scale
    if total_pixels > max_pixels:
        import math
        safe_scale = int(math.sqrt(max_pixels / (vp_width * vp_height)))
        safe_scale = max(safe_scale, 1)
        print(f"NOTE: Auto-reduced scale from {scale} to {safe_scale} (canvas {vp_width}x{vp_height} would exceed pixel limit at scale={scale})", file=sys.stderr)
        scale = safe_scale
    ...
    with sync_playwright() as p:
        ...
        # Cap SVG element size to avoid Playwright screenshot truncation on very large elements
        max_screenshot_pixels = 100_000_000  # 100MP safe limit
        svg_box = svg_el.bounding_box()
        if svg_box:
            css_w, css_h = svg_box["width"], svg_box["height"]
            screenshot_pixels = css_w * scale * css_h * scale
            if screenshot_pixels > max_screenshot_pixels:
                import math
                shrink = math.sqrt(max_screenshot_pixels / screenshot_pixels)
                new_w = int(css_w * shrink)
                new_h = int(css_h * shrink)
                page.evaluate(f"""(() => {{
                ...
                page.set_viewport_size({"width": max(new_w, 800), "height": max(new_h, 600)})
                svg_el = page.query_selector("#root svg")
                print(f"NOTE: Resized SVG from {int(css_w)}x{int(css_h)} to {new_w}x{new_h} to stay under screenshot pixel limit", file=sys.stderr)
